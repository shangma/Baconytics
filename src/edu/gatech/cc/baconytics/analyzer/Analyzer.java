...
public class Analyzer extends HttpServlet {
	...
	private JSONArray keywordListJarray = null;
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			...
			for (int i = 0; i < RETURN_THRESHHOLD; i++) {
				...
				kwJson.put("keyword", klMap.getKeyword());
				kwJson.put("link_list", linkJsonArray);
				keywordListJarray.put(kwJson);
		...
		for (User user : userList) {
			...
			try {
				JSONObject karmaJSON = new JSONObject();
				karmaJSON.put("rank", i);

				karmaJSON.put("value", user.getKarma());

				// JSONObject jsonKarma = new JSONObject();
				// jsonKarma.put("karma", karmaJSON);

				if (jsonUser.has(username)) {
					categoryJSON = jsonUser.getJSONObject(username);

				} else {
					categoryJSON = new JSONObject();
				}

				categoryJSON.put("karma", karmaJSON);
		...
		for (User user : userList) {
			...
			try {
				...
				visitsJSON.put("rank", i);

				visitsJSON.put("value", user.getTotalVisits());
				...
				categoryJSON.put("visits", visitsJSON);
				jsonUser.put(username, categoryJSON);
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
		...
		for (User user : userList) {
			String username = user.getUsername();
			try {
				JSONObject contriJSON = new JSONObject();
				contriJSON.put("rank", i);

				contriJSON.put("value", user.getTotalLinks());
				...
				categoryJSON.put("contribution", contriJSON);
				jsonUser.put(username, categoryJSON);
		...
		for (User user : userList) {
			...
			try {
				...
				commentJSON.put("rank", i);

				commentJSON.put("value", user.getTotalComments());
				...
				categoryJSON.put("comments", commentJSON);
				jsonUser.put(username, categoryJSON);
